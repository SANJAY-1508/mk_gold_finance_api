...
if (isset($obj['search_text'])) {
    ...
    $sql = "SELECT `id`, `sale_id`, `sale_date`, `name`, `place`, `mobile_number`, `bank_name`,
                   ...
                   `id_pic`, `id_pic_base64code`,`proof_pic`, `proof_pic_base64code`,
                    `staff_sign_pic`, `staff_sign_pic_base64code`,
                    `customer_sign_pic`, `customer_sign_pic_base64code`,
                    `delete_at`, `create_at`
            FROM `sale`
            WHERE `delete_at` = 0
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $row['proof_pic'] = json_decode($row['proof_pic'] ?? '[]', true) ?? [];
            $row['staff_sign_pic'] = json_decode($row['staff_sign_pic'] ?? '[]', true) ?? [];
            $row['customer_sign_pic'] = json_decode($row['customer_sign_pic'] ?? '[]', true) ?? [];
            ...
            $proofUrls = $convertToFullUrl($row['proof_pic']);
            $staffSignUrls = $convertToFullUrl($row['staff_sign_pic']);
            $customerSignUrls = $convertToFullUrl($row['customer_sign_pic']);
            ...
            $row['proof_pic'] = $proofUrls;
            $row['proof_pic_base64code'] = $proofUrls;

            $row['staff_sign_pic'] = $staffSignUrls;
            $row['staff_sign_pic_base64code'] = $staffSignUrls;

            $row['customer_sign_pic'] = $customerSignUrls;
            $row['customer_sign_pic_base64code'] = $customerSignUrls;
...
elseif (isset($obj['edit_sale_id'])) {
    ...
    $id_pic = $obj['id_pic'] ?? [];
    $proof_pic = $obj['proof_pic'] ?? [];
    $staff_sign_pic = $obj['staff_sign_pic'] ?? [];
    $customer_sign_pic = $obj['customer_sign_pic'] ?? [];
    ...
    $processBase64Images($proof_pic, $proofPaths, $proofBase64, "proof");
    $processBase64Images($staff_sign_pic, $staffSignPaths, $staffSignBase64, "staff_sign");
    $processBase64Images($customer_sign_pic, $customerSignPaths, $customerSignBase64, "customer_sign");
    ...
    $proofBase64Json = json_encode($proofBase64, JSON_UNESCAPED_SLASHES);
    $staffSignJson = json_encode($staffSignPaths, JSON_UNESCAPED_SLASHES);
    $staffSignBase64Json = json_encode($staffSignBase64, JSON_UNESCAPED_SLASHES);
    $customerSignJson = json_encode($customerSignPaths, JSON_UNESCAPED_SLASHES);
    $customerSignBase64Json = json_encode($customerSignBase64, JSON_UNESCAPED_SLASHES);
...
`customer_sign_pic`=?, `customer_sign_pic_base64code`=? 
    ...
    // FIXED: Added missing 6 JSON vars (proof, staff_sign, customer_sign) before $timestamp; now 24 vars total
    $stmt->bind_param(
        "ssssssssssssssssssssssss",
        $sale_date,
        $name,
